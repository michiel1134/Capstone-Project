...
tmap_mode("view")
tm_shape(G1) + tm_basemap("Stamen.Watercolor") + tm_fill("MPT", palette = "YlGn", title = "Markets/1000") + 
  tm_layout(legend.position = c(.87,.25),
            legend.text.size = .5, 
...
write.csv(All_Data_PCA, "Cleaned Data/All_Data_PCA.csv")
```
...
tmap_mode("view")
tm_shape(G2) + tm_basemap("Stamen.Watercolor") + tm_fill("clust",  palette = "Paired",   title = "Cluster") + 
  tm_layout(legend.position = c(.87,.25),
            legend.text.size = .5, 
            title.position = c("right","bottom"),
            title.fontface = "bold" ,
...
write.csv(Expected, "Cleaned Data/ExpectedFarmersMarkets.csv")
```
...
write.csv(E1, "Cleaned Data/E1.csv")
```
...
```{r}
FarmersMarketPredictions = left_join(All_Final_Data_Cleaned2, Expected %>% dplyr::select(-Freq), 
                          by = c("RowName"="County")) %>%
...
write.csv(FarmersMarketPredictions, "Cleaned Data/FarmersMarketPredictions.csv")
```
Now we have all the counties ranked in terms of their saturation, from most underserved to most saturated.  

###Proposed Actions
The "FarmersMarketPredictions.csv" table is meant as a starting point of research should an entrepreneur seek to start a farmers market without a location in mind.  It is suggested s/he look through the spreadsheet, which is ranked in order of most underserved, to find a county that may present itself.  S/he can go National Farmers Market Directory hosted by the USDA to find existing farmers markets in the area to determine a city/location.  If an entrepreneur should already have a location in mind, s/he can find the county on the aforementioned spreadsheet to compare the observed and expected number of farmers markets in that county.  Also, s/he can find other pertinent county level statistics that may assist in determining the viability of that county.

###Expected Benefits
This study has identified numerous counties that are underserved by farmers markets under the current conditions.  These communities, farmers and consumers alike, will likely benefit from more farmers markets.  Furthermore, with the vast amount of county level variables collected, new models can easily be created that analyze different factors.  For example, one can easily estimate the effect of political affiliation on farmers markets.  Lastly, this methodology is flexible and can be used to model any other count variable at the county level.  For example, one can model the number of restaurants by county of a large restaurant chain and identify potentially fruitful markets.
